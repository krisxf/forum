...
import com.kris.acg.common.FileFormatConstants;
...
public class ImageUtil {
    ...
    public static InputStream compressAsFixedWebpImage(MultipartFile multipartFile, int width, int height, int limitSize) {
        try {
            ...
            BufferedImage bufferedImage = Thumbnails.of(originalImage)
                    ...
                    .outputFormat(FileFormatConstants.JPG)
                    .asBufferedImage();
            try (InputStream is = ImageUtil.bufferedImage2InputStream(bufferedImage, FileFormatConstants.JPG)) {
    ...
    public static InputStream compressBak(MultipartFile multipartFile, int limitSize) {
        ...
        String format = FileUtil.formatOf(multipartFile);
        try {
...
//            imageBuilder.size(270, 260).outputQuality(1f);
            return ImageUtil.bufferedImage2InputStream(imageBuilder.asBufferedImage(), FileUtil.formatOf(multipartFile));
        } catch (IOException e) {
            log.error(e.getMessage());
